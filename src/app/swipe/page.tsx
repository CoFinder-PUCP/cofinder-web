  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <div className="max-w-lg mx-auto px-4 py-8 flex flex-col gap-4">
        <h1 className="text-xl font-semibold">Explorar startups</h1>
        ...
        {isLoading && (
          <p className="text-muted-foreground text-sm">Cargando startups...</p>
        )}

        {!isLoading && !current && (
          <div className="text-center py-16 flex flex-col gap-4 items-center">
            <p className="text-muted-foreground text-sm">No hay más startups por ahora.</p>
            <Button
              variant="outline"
              onClick={() => {
                setCurrentIndex(0);
        ...
        {current && (
          <Card>
            <CardContent className="pt-6 flex flex-col gap-4">
              ...
              <p className="text-xs text-muted-foreground font-medium">{current.category}</p>
              ...
              {current.budget != null && (
                <p className="text-sm text-muted-foreground">
                  Presupuesto: ${current.budget.toLocaleString()}
                </p>
              )}
              ...
              <div className="flex gap-3 pt-2">
                <Button
                  ...
                  className="flex-1"
                  disabled={isPending}
                  onClick={() => swipe({ startupId: current.id, direction: 'LEFT' })}
                >
                  Pasar
                </Button>
                <Button
                  className="flex-1"
                  disabled={isPending}
                  onClick={() => swipe({ startupId: current.id, direction: 'RIGHT' })}
                >
                  Aplicar
                </Button>
              </div>
